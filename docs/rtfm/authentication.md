...
### CanCanCan
If you are using [CanCanCan](https://github.com/CanCanCommunity/cancancan) for authorization, you'll need to use different strategies for _page morphs_ vs _selector morphs_. 
...
##### Option 1: delegate current_ability to controller
```ruby
...
##### Option 2: create new ability for user
```ruby
class ClassroomsReflex < ApplicationReflex
  delegate :current_user, to: :connection
